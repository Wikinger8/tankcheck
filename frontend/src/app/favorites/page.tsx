  ...
  return (
    <main className="min-h-screen bg-[#0c0c0f] pb-24">
      <div className="mx-auto max-w-lg px-4 pt-6">
        <h1 className="text-2xl font-bold text-white mb-1">{t('favorites.title')}</h1>
        <p className="text-sm text-[#555566] mb-6 tabular-nums">{favorites.length} gespeichert</p>

        {loading && favorites.length > 0 && (
          <div className="space-y-3">
            {Array.from({ length: Math.min(favorites.length, 3) }).map((_, i) => (
              <div key={i} className="rounded-xl bg-[#141418] border border-[#2a2a34] p-4 animate-pulse">
                <div className="h-4 bg-[#1c1c22] rounded w-1/3 mb-2" />
                <div className="h-3 bg-[#1c1c22] rounded w-2/3" />
              </div>
            ))}
          </div>
        )}
        ...
        {favorites.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <h2 className="text-lg font-bold text-white mb-1">{t('favorites.empty')}</h2>
            <p className="text-sm text-[#555566] max-w-[260px]">{t('favorites.emptyDesc')}</p>
          </div>
        )}
      </div>
    </main>
